...
function roleIdFromPositionId(positionId: string | null): string | null {
  if (!positionId) return null
  if (positionId === 'ketum' || positionId === 'waketum') return 'super_admin'
  if (positionId === 'sekum_1' || positionId === 'bendum_1') return 'admin_organization'
  if (
  ...
  if (positionId === 'sekum_2' || positionId === 'bendum_2') return null
  if (positionId.startsWith('kadep_')) return null
  if (positionId.startsWith('sekdep_')) return null
  if (positionId.startsWith('anggota_')) return null
  throw new Error(`Position belum memiliki mapping role: ${positionId}`)
}

function ensureRoleMatchesPosition(user: { name: string; roleId: string | null; positionId: string | null }) {
...
const roles = [
  ...
  { id: 'admin_organization', name: 'Admin Organisasi', description: 'Pengelola data operasional organisasi sesuai permission' },
...
const permissions = [
  ...
  { id: 'structure.manage', name: 'Manage Organization Structure', module: 'Organization' },
  ...
  { id: 'document_archive.view', name: 'View Document Archives', module: 'Documents' },
  { id: 'document_archive.manage', name: 'Manage Document Archives', module: 'Documents' },
...
// Development accounts intentionally keep only role and organizational mappings
// from master data. Their identifiers, names, email addresses, and media are
// synthetic so this seed cannot recreate real organization accounts.
const developmentUsers = masterDataSeed.members.map((member, index) => {
  const sequence = String(index + 1).padStart(2, '0')

  return {
    id: `dev-user-${sequence}`,
    name: `Development Member ${sequence}`,
    email: `dev-member-${sequence}@example.test`,
...
async function main() {
  const developmentSeedPassword = assertDevelopmentSeedAllowed()
  ...
  await prisma.user.updateMany({
    where: { roleId: { in: ['admin_sekretaris', 'admin_bendahara'] } },
    data: { roleId: 'admin_organization', sessionVersion: { increment: 1 } },
  })

  await prisma.user.updateMany({
    where: { roleId: { in: [...deprecatedRoleIds, 'user'] } },
    data: { roleId: null, sessionVersion: { increment: 1 } },
  })
  ...
  const adminPasswordHash = await bcrypt.hash(developmentSeedPassword, 10)
  const memberPasswordHash = await bcrypt.hash(developmentSeedPassword, 10)
  ...
  for (const member of developmentUsers) {
    ...
    JSON.stringify(
      {
        ...
        members: developmentUsers.length,
