...
export async function createLetterAction(data: unknown) {
  try {
    ...
    if (!session?.user || !(await can('letter.manage', { id: session.user.id!, roleId: (session.user as { roleId: string }).roleId, departmentId: session.user.departmentId! }))) {
      return { error: 'Akses ditolak.' }
    }

    const sessionUser = { id: session.user.id!, roleId: (session.user as { roleId: string }).roleId, departmentId: session.user.departmentId! }
