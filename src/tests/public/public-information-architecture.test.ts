...
describe('public PRD information architecture', () => {
  it('keeps the homepage editorial sequence on current public routes', () => {
    ...
    expect(home).toContain('href={homepageContent.profile.ctaHref}')
    ...
    expect(home).toContain('href={homepageContent.cta.href}')
    expect(home).toContain('getPublicAgendaOccurrences')
    expect(home).toContain('=== "AKAN_DATANG"')
    expect(home).toContain('const featuredCampaign = activeCampaigns[0] ?? null')
    expect(home).toContain('campaignDesktopImage')
    expect(home).toContain('campaignMobileImage')
    expect(home).toContain('campaignPhasePresentation')
    expect(home).toContain('.sort((left, right) =>')
    expect(home).toContain('.slice(0, 3)')
    expect(home).not.toContain('getPublicPrograms')
    expect(home).not.toContain('deriveProgramStatus')
    expect(home).not.toContain('Program terbaru akan ditampilkan')
