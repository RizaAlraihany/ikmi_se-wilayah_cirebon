                    ...
                    {posts.map((post) => (
                      <tr key={post.id}>
                        <td className="px-4 py-4">
                          ...
                          <p className="mt-1 text-xs text-muted">{post.category?.name || '-'}</p>
                        </td>
                        <td className="px-4 py-4 text-muted">{post.author?.name || '-'}</td>
                        <td className="px-4 py-4 text-muted">{post.author?.department?.name || '-'}</td>
                        <td className="px-4 py-4"><Badge tone={post.status === 'PUBLISHED' ? 'success' : post.status === 'ARCHIVED' ? 'surface' : post.status === 'APPROVED' ? 'accent' : 'warning'}>{post.status}</Badge></td>
                        <td className="px-4 py-4 text-right">
                          <ButtonLink href={`/admin/cms/posts/${post.id}`} variant="ghost" size="icon" aria-label={`Edit ${post.title}`}>
                  ...
                  <Card key={post.id}>
                    <CardContent className="space-y-3 p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge tone={post.status === 'PUBLISHED' ? 'success' : post.status === 'ARCHIVED' ? 'surface' : post.status === 'APPROVED' ? 'accent' : 'warning'}>{post.status}</Badge>
                      </div>
                      <p className="text-sm text-muted">{post.category?.name || '-'} - {post.author?.name || '-'} - {post.author?.department?.name || '-'}</p>
                      <ButtonLink href={`/admin/cms/posts/${post.id}`} variant="secondary" size="sm">
                        <Edit className="h-4 w-4" aria-hidden="true" />
                        Edit
                      </ButtonLink>
                    </CardContent>
