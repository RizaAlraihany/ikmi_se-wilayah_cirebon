...
export function ProgramEditor({ program, units, periods, members }: { program: Program; units: Unit[]; periods: Period[]; members: Member[] }) {
  ...
  async function submit(formData: FormData) {
    ...
    setMessage(result.error ?? 'Data kegiatan diperbarui.')
    setSaving(false)
    if (result.success) router.refresh()
  }

  async function archive() {
    if (!window.confirm('Arsipkan Kegiatan ini? Data tidak dihapus permanen.')) return
    setSaving(true)
