...
import { Building2, Plus, UserRoundCog } from 'lucide-react'
import {
...
type Period = { id: string; name: string; status: 'DRAFT' | 'ACTIVE' | 'ARCHIVED' }
type Unit = {
  ...
  return (
    <div className="space-y-6">
      <header className="border-y-2 border-primary bg-surface px-1 py-6 sm:px-5">
        ...
        <h1 className="mt-2 font-heading text-3xl font-extrabold text-balance text-primary">Profil Organisasi</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-pretty text-text-secondary">Kelola unit dan jabatan organisasi. Pergantian periode dilakukan hanya dari Pengaturan agar histori kepengurusan tetap aman.</p>
      </header>
      ...
      <section className="border-l-2 border-primary bg-surface-alt px-4 py-3 text-sm leading-6 text-text-secondary">
        Periode aktif dan histori dapat ditinjau dari <Link href="/admin/organization/settings" className="font-bold text-primary underline underline-offset-4">Pengaturan Organisasi</Link>. Data kabinet periode aktif dikelola dari CMS Tentang.
      </section>

      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Building2 className="size-5" aria-hidden="true" />Unit organisasi</CardTitle></CardHeader>
        <CardContent className="grid gap-4 lg:grid-cols-2">
          ...
          <details className="border border-dashed border-border bg-surface-alt p-4"><summary className="cursor-pointer font-semibold text-primary">Tambah unit organisasi</summary><form action={createUnit} className="mt-4 grid gap-3"><UnitFields periods={periods} members={members} disabled={isSaving} /><Button type="submit" disabled={isSaving}><Plus className="size-4" aria-hidden="true" />Tambah unit</Button></form></details>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><UserRoundCog className="size-5" aria-hidden="true" />Tambah jabatan</CardTitle></CardHeader>
