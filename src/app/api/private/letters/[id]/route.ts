...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const user = await requirePermission('letter.view')
    ...
    if (!letter?.filePublicId) return NextResponse.json({ error: 'File tidak ditemukan' }, { status: 404 })

    const response = NextResponse.redirect(storageService.getPrivateDocumentUrl(letter.filePublicId))
