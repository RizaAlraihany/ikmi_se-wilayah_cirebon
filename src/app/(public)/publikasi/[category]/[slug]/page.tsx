...
export async function generateMetadata({ params }: { params: Promise<{ category: string; slug: string }> }) {
  ...
  if (!post) {
    return { title: 'Publikasi Tidak Ditemukan', robots: { index: false, follow: false } }
  }

  const canonicalPath = `/publikasi/${post.category.slug}/${post.slug}`
...
export default async function BlogDetailPage({ params }: { params: Promise<{ category: string; slug: string }> }) {
  ...
  if (category !== post.category.slug) permanentRedirect(`/publikasi/${post.category.slug}/${post.slug}`)
  ...
  const articleJsonLd = serializeStructuredData({
    ...
    '@graph': [
      {
        '@type': 'Article',
        mainEntityOfPage: `${siteUrl}/publikasi/${post.category.slug}/${post.slug}`,
        headline: post.title,
      ...
      breadcrumbStructuredData([
        ...
        { name: post.title, path: `/publikasi/${post.category.slug}/${post.slug}` },
      ]),
    ],
  })
            ...
            <PublicBreadcrumb
              items={[
                { label: 'Publikasi', href: '/publikasi' },
                {
                  ...
                  href: `/publikasi?category=${post.category.slug}`,
                },
                { label: post.title },
              ]}
            />
          ...
          <div className="grid grid-cols-1 gap-8 md:gap-10 lg:grid-cols-12 lg:items-start">
            <article className="space-y-5 lg:col-span-8 lg:max-w-[760px]">
              ...
              <div className="flex flex-col gap-4 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <ButtonLink
                  href="/publikasi"
                  variant="ghost"
                  className="min-h-11 px-0 text-xs font-extrabold uppercase tracking-wide text-primary hover:bg-transparent hover:text-accent"
                >
                  Halaman Depan Blog
            ...
            <aside className="space-y-6 lg:col-span-4">
                ...
                <div className="rounded-lg border border-border bg-surface p-5 shadow-card">
                  ...
                  <div className="space-y-4">
                    {relatedPosts.map((item) => (
                      <Link
                        key={item.id}
                        href={`/publikasi/${item.category.slug}/${item.slug}`}
                      ...
                      >
                        ...
                        <div className="min-w-0 space-y-1">
                          ...
                          <h3 className="text-xs font-bold leading-snug text-primary transition-colors group-hover:text-accent">
                            {item.title}
                          </h3>
                        </div>
                      </Link>
              ...
              <div className="overflow-hidden rounded-lg bg-primary p-5 text-text-inverse shadow-card">
                <span className="inline-flex rounded-md bg-surface/15 px-3 py-1 text-[9px] font-extrabold uppercase tracking-widest">
                  Journal Newsroom
                </span>
                ...
                <ButtonLink
                  href="/publikasi"
                  variant="secondary"
                  className="mt-5 min-h-11 bg-surface px-4 text-xs text-primary hover:bg-surface-alt"
                >
                  Lihat Semua Artikel
                  <ChevronRight className="h-4 w-4" aria-hidden="true" />
                </ButtonLink>
