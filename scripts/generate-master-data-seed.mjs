...
const bphPositionMap = {
  ...
  'wakil ketua umum': { roleId: 'super_admin', departmentId: 'BPH', positionId: 'waketum' },
  'sekretaris umum i': { roleId: 'admin_sekretaris', departmentId: 'BPH', positionId: 'sekum_1' },
  'sekretaris umum ii': { roleId: 'user', departmentId: 'BPH', positionId: 'sekum_2' },
  'bendahara umum i': { roleId: 'admin_bendahara', departmentId: 'BPH', positionId: 'bendum_1' },
  'bendahara umum ii': { roleId: 'user', departmentId: 'BPH', positionId: 'bendum_2' },
}

const bphCanonicalNames = {
  ketum: 'Raden Hafidz Rahmatullah',
  waketum: 'Muhamad Walid',
  sekum_1: 'Soliha',
  sekum_2: 'Atinah',
  bendum_1: 'Hallimatul Syahdiyah',
  bendum_2: 'Uswatun Hasanah',
}

const departmentPositionRules = [
  { prefix: 'ketua departemen', positionPrefix: 'kadep' },
  { prefix: 'sekretaris departemen', positionPrefix: 'sekdep' },
  { prefix: 'anggota', positionPrefix: 'anggota' },
]
...
function getHeaderMap(headerRow = []) {
  return new Map(headerRow.map((value, index) => [headerKey(value), index]))
}

function getColumn(row, headerMap, names, fallbackIndex) {
...
function normalizeOptionalUrl(value, fieldName, context) {
  ...
  try {
    ...
    throw new Error(`${fieldName} harus berupa URL valid untuk ${context}: ${text}`)
  }

  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error(`${fieldName} harus menggunakan http/https untuk ${context}: ${text}`)
  }
...
function cloudinaryPublicIdFromUrl(url) {
  ...
  const parts = decodeURIComponent(parsed.pathname).replace(/^\/+/, '').split('/')
  const uploadIndex = parts.indexOf('upload')
  if (uploadIndex === -1) return null

  const publicParts = parts.slice(uploadIndex + 1)
  while (publicParts[0] && /^v\d+$/.test(publicParts[0])) {
    publicParts.shift()
  }
  ...
  publicParts.push(filename.replace(/\.[^.]+$/, ''))
...
function resolvePosition(jabatan) {
  ...
  if (rule) {
    const positionId = `${rule.positionPrefix}_${departmentId.toLowerCase()}`
    return {
      roleId: departmentId === 'KOMDIGI' ? 'admin_komdigi' : 'user',
      departmentId,
      positionId,
    }
  }
...
function canonicalPositionName(position, fallback) {
  ...
  if (position.positionId?.startsWith('kadep_')) return `Ketua Departemen ${departmentName}`
  if (position.positionId?.startsWith('sekdep_')) return `Sekretaris Departemen ${departmentName}`
  if (position.positionId?.startsWith('anggota_')) return `Anggota Departemen ${departmentName}`
...
function buildData(tables) {
  ...
  const memberRows = memberTable.slice(1)
  ...
  const members = memberRows.map((row, index) => {
    const name = normalizeText(getColumn(row, memberHeaders, ['NAMA', 'NAME', 'NAMA LENGKAP'], 1))
    const jabatan = normalizeText(getColumn(row, memberHeaders, ['JABATAN', 'POSITION'], 2))
    const position = resolvePosition(jabatan)
    const canonicalName = bphCanonicalNames[position.positionId] ?? name
    const photoUrl = normalizeOptionalUrl(getColumn(row, memberHeaders, ['PHOTO', 'FOTO', 'PHOTO URL'], 7), 'PHOTO', canonicalName)

    return {
      id: `member_${String(index + 1).padStart(2, '0')}_${slug(canonicalName)}`,
      name: canonicalName,
      email: normalizeEmail(getColumn(row, memberHeaders, ['EMAIL', 'E-MAIL'], 6), index + 1),
      sourceEmail: normalizeText(getColumn(row, memberHeaders, ['EMAIL', 'E-MAIL'], 6)),
      positionName: canonicalPositionName(position, jabatan),
      birthInfo: normalizeText(getColumn(row, memberHeaders, ['TTL', 'TEMPAT TANGGAL LAHIR'], 3)),
      phone: normalizeText(getColumn(row, memberHeaders, ['NO HP', 'NO. HP', 'PHONE', 'WHATSAPP'], 4)),
      address: normalizeText(getColumn(row, memberHeaders, ['ALAMAT', 'ADDRESS'], 5)),
      photoUrl,
      photoPublicId: cloudinaryPublicIdFromUrl(photoUrl),
      ...position,
    }
  })
  ...
  JSON.stringify(
    {
      ...
      photos: data.members.filter((member) => member.photoUrl).length,
